...
import 'package:salon/pages/auth/authentication.dart';
...
@override
  ...
  _setInitialScreen(User? user) {
    if (user==null) {
      Get.offAllNamed(Routes.authentication);    } else {
      // ignore: unrelated_type_equality_checks
      if(CacheHelper.get(key: 'role') == 'Salon Owner'){
  ...
  void onCreateAccount() async {
    try {
      ...
      CacheHelper.put(key: 'role', value: selected.value);
...
update();
      ...
      UserModel user1 = UserModel(
          ...
          phone: phone.text,
          uid: cred.user!.uid,
         password: password.text, role: selected.value);
